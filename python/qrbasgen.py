...
def write_file(script_lines: list[str]) -> None:
	temp_dir = pathlib.Path(tempfile.gettempdir()).joinpath('meik', 'qr_code', 'basic_script')
	temp_dir.mkdir(parents=True, exist_ok=True)
	temp_file = tempfile.NamedTemporaryFile(mode='w', encoding="latin-1", dir=str(temp_dir), delete=False)
	temp_file.writelines(script_lines)
	temp_file.close()
